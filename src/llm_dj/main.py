...
class LLMDJ:
    ...
    async def create_playlist_from_prompt(self, prompt, playlist_name: Optional[str] = None):
        ...
        print(f"[bold green]Suggestions:[/bold green]")
        pprint(suggestions)
        ...
        playlist = self.spotify.create_playlist(playlist_name, description=f"{suggestions.description}.\nGenerated by LLM-DJ from prompt: {prompt}")
        ...
        for song in suggestions.suggestions:
            ...
            print(f"[bold green]Searching for ({song.type_}): {song.name}[/bold green]")
            if song.type_ == "song":
